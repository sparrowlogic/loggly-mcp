...
public class LogglyTools {
	...
	public SearchResult searchLogs(
			...
			Integer pageSize) {
		SearchCacheService.Handle handle = cache.search(query, from, until, order);
		return page(handle.search(), 0, pageSize(pageSize), handle.reused());
	}
	...
	public SearchResult fetchPage(
			...
			@McpToolParam(description = "Events per page (default 25, max 200). Optional.",
					required = false)
			Integer pageSize) {
		CachedSearch snapshot = cache.get(searchId);
		return page(snapshot, page == null ? 0 : Math.max(0, page), pageSize(pageSize), true);
	}
	...
	public JsonNode listFields(
			...
			@McpToolParam(description = "Number of fields to return (default 10, max 500).",
					required = false)
			Integer facetSize) {
		return client.listFields(query, from, until, facetSize == null ? 0 : facetSize);
	}
	...
	public JsonNode fieldValues(
			...
			@McpToolParam(description = "Number of values to return (default 50, max 300).",
					required = false)
			Integer facetSize) {
		return client.fieldValues(field, query, from, until, facetSize == null ? 0 : facetSize);
	}
	...
	private SearchResult page(CachedSearch s, int page, int pageSize, boolean cached) {
		...
		List<CompactEvent> events = s.events().subList(start, end).stream()
				.map(e -> CompactEvent.from(e, properties.messageMaxChars()))
				.toList();
